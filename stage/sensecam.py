...
def newest_frame(sense_dir, now=None):
    ...
    best = None
    for slot in _slot_names(sense_dir):
        slot_dir = os.path.join(sense_dir, slot)
        try:
        ...
        for name in names:
            if not _is_frame_name(name):
                continue
            target = data.contained_file(sense_dir, os.path.join(slot_dir, name))
            if target is None:
                continue
            try:
            ...
            if best is None or mtime > best[2]:
                best = (slot, name, mtime)
    if best is None or now - best[2] > FRESH_SECONDS:
        return None
    return {"slot": best[0], "name": best[1], "captured_epoch": best[2]}


def frame_bytes_path(sense_dir, slot, name):
    ...
    """
    if slot not in _slot_names(sense_dir):
        return None
    slot_dir = os.path.join(sense_dir, slot)
    try:
        names = os.listdir(slot_dir)
