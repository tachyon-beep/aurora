...
**Amended 2026-08-17: the seed is generated, not kept in agreement.** The static seed above drifted
exactly as this section predicted, and the agreement test did not catch it: the test compares the
seed against `.env.example`, so it stayed green while the live `.env` — which had replaced
`deepseek/deepseek-v4-flash` with `z-ai/glm-5.2` — bound two of three text sockets, the third
rejected with "model not permitted". A test that can only see the shipped default cannot protect
the deployment that is actually running.

`llm_console_seed.json` is now a generated, gitignored artifact, built by
`scripts/build_console_seed.py` from `.env` (falling back to `.env.example` for a clone that has
none) and regenerated by `scripts/prepare_host.sh` alongside `garden_export/`. Seed and allow lists
agree by construction rather than by assertion, for the operator's own lists rather than only the
shipped ones. Naming, per-stream values, and the at-most-`MAX_STREAMS` rule are unchanged, except
that exceeding the limit is now a build-time refusal rather than a silent rejection inside the
recorder. Per-stream `budget`/`token_budget` stay fixed rather than tracking
`STREAM_HOURLY_MAX`/`STREAM_TOKEN_HOURLY_MAX`, and are deliberately at or above the ceilings the
template recommends: a declared budget only ever lowers (`effective_allowance` takes the minimum),
and the entrypoint seeds the console once and never again, so a value generated against today's
ceiling would outlive it and cap a later, higher one. Overstating the budget is a no-op the
ceiling absorbs; understating it is permanent. Generation stays on the host: `/llm/console` is
written by the agent and read-only to the recorder, so no container-side component may produce it.
